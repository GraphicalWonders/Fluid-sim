...
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mods) {
    ...
    // updates global state based on key presses for real-time interaction (exit or record)
}
...
void saveFrame(int frameNumber) {
    ...
    glPixelStorei(GL_PACK_ALIGNMENT, 1); // ensure correct alignment of pixel rows
    ...
    // opengl's coordinate system starts at the bottom-left, so we flip the image vertically
...
int main() {
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        // recording mode saves each frame to a file, later combined by ffmpeg.
        if (g_recording) {
